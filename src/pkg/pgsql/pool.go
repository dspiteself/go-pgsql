	...
	for p != nil && !p.closed {
		...
		for p.conns.Len() > 0 {
			...
			if (now.Sub(atime)) > p.timeout {
				...
				delay = 1e9 * (p.timeout - (now.Sub(atime)) + 1)
				break
			}
		}
...
// closed.
func NewPool(connectParams string, minConns, maxConns, idleTimeout int) (p *Pool, err error) {
	if minConns < 1 {
	...
	p = &Pool{
		&pool{
			...
			timeout: time.Duration(idleTimeout),
		},
	}
	p.conns.PushFront(poolConn{c, time.Now()})
